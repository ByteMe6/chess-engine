...
- [Как подключить библиотекой](#-kak-podkliuchit)
...
<a id="-kak-podkliuchit"></a>
## 📦 Как подключить библиотекой

Весь движок — один заголовочный файл. Выбери свой вариант и просто `#include <chessEngine.hpp>`.

**CMake FetchContent** — для минималистов, без пакетного менеджера:
...
**vcpkg** — через [собственный реестр](https://github.com/ByteMe6/vcpkg-registry). В `vcpkg-configuration.json` твоего проекта:
...
дальше добавь `"chess-engine"` в зависимости своего `vcpkg.json`. В репозитории также лежит [overlay-порт](ports/chess-engine):
...
**Conan** — из клона этого репозитория:
...
```ini
# дальше в conanfile.txt твоего проекта
[requires]
chess-engine/1.1.0
```
...
- ✅ **Рокировка шахмат Фишера (960)** — король и ладьи могут стартовать где угодно на первой горизонтали; рокировка ходом «король берёт свою ладью», поддерживаются права в формате Shredder-FEN (`HAha`)
- ✅ **Правила ничьей** — правило 50 ходов, троекратное повторение позиции и недостаток материала заканчивают партию `Game over: draw`
- ✅ **Превращение пешки** — пешка на последней горизонтали становится тем, что передашь в `promote()`; играй дальше без него — и по умолчанию будет ферзь
...
board.makeMove("a7", "a8");               // пешка дошла до последней горизонтали...
board.promote(FigureName::Knight);        // ...выбери фигуру или пропусти ради ферзя
```
...
| `promote` | `void promote(FigureName name)` | Выбирает фигуру для превращения; пропусти — и пешка станет ферзём следующим ходом |
...
├── tests/
│   └── tests.cpp         # тесты — запуск через ctest
├── bin/
...
└── CMakeLists.txt        # демо + тесты + библиотека chess-engine::chessEngine
```
...
- [x] Взятие на проходе
- [x] Превращение пешки
- [x] Правила ничьей (повторение, 50 ходов, недостаток материала)
- [x] Импорт/экспорт FEN
- [x] Рокировка шахмат Фишера (960)
- [x] Тесты
...
ctest --test-dir build    # тесты (должны оставаться зелёными)
